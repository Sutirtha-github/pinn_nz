...
class NZSolverEx:
    def __init__(self, D=1, steps=500, A=0.0112, omega_c=3.04, T=10, device='cuda' if torch.cuda.is_available() else 'cpu'):
        ...
        # Define operators
        self.Mx = torch.tensor([[0, -1], [-1, 0]], dtype=torch.cfloat, device=device)
        self.Mz = torch.tensor([[1, 0], [0, -1]], dtype=torch.cfloat, device=device)
        self.S = torch.tensor([[0, 0], [0, 1]], dtype=torch.cfloat, device=device)

        self.om_b = torch.pi / self.D
        self.Hs = self.rabi_const(self.om_b)/2 * self.Mx
    ...
    def solve(self, rho_init):
        ...
        for i, t in tqdm(enumerate(times), total=len(times), desc='Solving Nakajima-Zwanzig EOM for Exciton model'):
    ...
    def plot_results(self, times, rhos):
        ...
        plot_numerical(times_cpu, rho_diag_cpu, total_cpu, self.D, sys='e')